...
// Function to fetch budget data for a given email from the database
export default async function getBudgetData(email: string) {
  ...
  if (!email) {
    ...
    return { success: false, data: [] as BudgetFetchData[] }; // Return failure response with empty data
  }

  try {
    // Attempt to fetch all budget entries from the database where the email matches
